...
def do_sign_covariant_pca(X, n_components):
    ...
    U, S, V = randomized_svd(X_normalized, n_components=n_components, flip_sign=True)
...
def do_pca_step(features, n_components, normalize = True, epsilon = 1e-8):
    ...
    features = np.reshape(features, [shape_initial[0], shape_initial[2],
                                     shape_initial[3], features.shape[-1]])
