...
class InvariantsPurifier:
    def __init__(self, regressor = DEFAULT_LINEAR_REGRESSOR, num_to_fit = '10x'):
    ...
    def fit(self, old_blocks, new_block):
        if (type(self.num_to_fit_) is str):
            ...
            num_fit_now = multiplier * new_block.shape[1]
        else:
            num_fit_now = self.num_to_fit_
            
        if (num_fit_now > new_block.shape[0]):
            warnings.warn("Amount of provided data is less than the desired one to fit InvariantsPurifer. Number of new features is {}, desired number of environments is {}, actual number of environments is {}".format(new_block.shape[1], num_fit_now, new_block.shape[0]))
...
class CovariantsIndividualPurifier:
    ...
    def fit(self, old_blocks, new_block, l):
        ...
        if (num_fit_now > new_block.shape[0] * (l + 1)):
            warnings.warn("Amount of provided data is less than the desired one to fit InvariantsPurifer. Number of new features is {}, desired number of data points is {}, actual number of data points (n_env * (l + 1)) is {}, since number of environments is {}, and l is {}".format(new_block.shape[1], num_fit_now, new_block.shape[0] * (l + 1), new_block.shape[0], l)) 
...
class CovariantsPurifier:
    ...
    def fit(self, old_datas, new_data):
        ...
        for l in range(self.l_max_ + 1):
            self.purifiers_.append(CovariantsIndividualPurifier(regressor = clone(self.regressor_), num_to_fit = self.num_to_fit_)) 
...
class CovariantsPurifierBoth:
    def __init__(self, regressor = DEFAULT_LINEAR_REGRESSOR, num_to_fit = '10x'):
        self.num_to_fit_ = num_to_fit
        self.even_purifier_ = CovariantsPurifier(regressor = clone(regressor), num_to_fit = self.num_to_fit_)
        self.odd_purifier_ = CovariantsPurifier(regressor = clone(regressor), num_to_fit = self.num_to_fit_)
        self.fitted_ = False
