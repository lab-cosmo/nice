...
class InvariantsPurifier:
    ...
    def fit(self, old_blocks, new_block):
        ...
        if (num_fit_now > new_block.shape[0]):
            warnings.warn(
                "Amount of provided data is less than the desired one to fit InvariantsPurifer. Number of old features is {}, desired number of environments is {}, actual number of environments is {}"
                .format(total_num, num_fit_now, new_block.shape[0]))
...
class CovariantsIndividualPurifier:
    ...
    def fit(self, old_blocks, new_block, l):
        ...
        if (num_fit_now > new_block.shape[0] * (l + 1)):
            warnings.warn(
                "Amount of provided data is less than the desired one to fit InvariantsPurifer. Number of old features is {}, desired number of data points is {}, actual number of data points (n_env * (l + 1)) is {}, since number of environments is {}, and l is {}"
                .format(total_num, num_fit_now, new_block.shape[0] * (l + 1),
                        new_block.shape[0], l))
